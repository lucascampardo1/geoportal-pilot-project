...
function mostrarAlertaDeCarregamento(mensagem) {
    const mapaContainer = document.getElementById('map');
    if (!mapaContainer || document.getElementById('aviso-camadas'))
        return;
...
async function initMap() {
    ...
    if (camadasComErro.length > 0) {
        mostrarAlertaDeCarregamento(`Não foi possível carregar: ${camadasComErro.join(', ')}`);
    }
}
initMap();
